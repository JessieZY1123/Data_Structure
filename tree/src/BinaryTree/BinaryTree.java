...
public abstract class BinaryTree extends Tree{
	public BinaryTree() {
		super();//extends the parent's constructor
	}
	...
	public void addLeft(BinaryNode node, BinaryNode newChild) throws Exception{
		if(node.getLeft()!=null)
			throw new Exception("Left child already exits");
	...
	public void addRight(BinaryNode node, BinaryNode newChild) throws Exception{
		if(node.getRight()!=null)
			throw new Exception("Right child already exits");
		node.setRight(newChild);
		newChild.setParent(node);
		size++;
	}
	//removes a leaf but promotes and removes a descendant 
	...
	public ArrayList<TNode> flatOrdrt(){
		return inOrder();
	}
}
